# Sprite Tools UI Toolkit Rewrite — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the IMGUI Sprite Tools batch pivot/naming window with an idiomatic UI Toolkit (UXML + USS + C#) window, leaving the UI-agnostic Core untouched.

**Architecture:** A code-instantiated `EditorWindow` loads a UXML layout and a USS stylesheet by asset path. Static controls bind to the window's `[SerializeField]` fields via `SerializedObject` (`binding-path`); the dynamic per-row list is rendered by hand from a serializable `RowConfig` model rebuilt on selection. A single `Refresh()` synchronizes all derived view state (mode banner, section visibility/enable, rows, live preview). Apply orchestration and all asset mutations are ported verbatim from the IMGUI window and continue to call `SpriteImportOps` / `SpriteSheetRows` / `SpriteNaming`.

**Tech Stack:** Unity 2022.3.62f2, UI Toolkit (`UnityEngine.UIElements`, `UnityEditor.UIElements`), C# (Assembly-CSharp-Editor, no asmdef).

---

## Important execution notes

- **No auto-commit.** Per project preference, do **not** commit. Create files, compile-check, then hand back to the user to import in Unity and commit. The final task lists the Unity Editor steps and leaves the commit to the user.
- **`.meta` files are generated by Unity**, not by this plan. New `.cs`/`.uxml`/`.uss` files will have no `.meta` until the user focuses the Unity Editor to trigger an import. Do not hand-write `.meta` files (GUIDs must come from Unity).
- **No automated tests are added.** The testable Core is unchanged, and `EditorWindow` view logic is not meaningfully unit-testable. The per-task gate is an **offline compile-check** (Unity's bundled Roslyn against a cloned response file); final validation is a **manual smoke test** in the Editor.
- **Faithful behavior.** Mode detection, Apply ordering (pivot before rename), and all Core calls match the current tool. The only intended changes are UI-layer polish: live-updating preview and reactive enable-states (disabled sections stay visible but greyed).

### Reusable offline compile-check (referenced by several tasks)

Run from repo root `F:/Work/unity/piratik/PlatformStudy` in the Bash tool:

```bash
cd "F:/Work/unity/piratik/PlatformStudy"
rsp=$(ls -t Library/Bee/artifacts/*.dag/Assembly-CSharp-Editor.rsp | head -1)
mkdir -p Temp/obcompile
# Clone the response file: redirect outputs to a throwaway path, drop the deleted
# old window, add the new window source.
sed -e 's#^-out:.*#-out:"Temp/obcompile/out.dll"#' \
    -e 's#^-refout:.*#-refout:"Temp/obcompile/out.ref.dll"#' \
    -e '\#SpritePivotBatchTool\.cs#d' \
    "$rsp" > Temp/obcompile/check.rsp
echo '"Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.cs"' >> Temp/obcompile/check.rsp
U="C:/Program Files/Unity/Hub/Editor/2022.3.62f2/Editor/Data"
MSYS2_ARG_CONV_EXCL="*" "$U/NetCoreRuntime/dotnet.exe" exec "$U/DotNetSdkRoslyn/csc.dll" \
    -nostdlib -noconfig "@Temp/obcompile/check.rsp"
```

Notes:
- `-noconfig`/`-nostdlib` must be on the command line, not in the `.rsp`.
- `MSYS2_ARG_CONV_EXCL="*"` stops git-bash from mangling `-`/`/` flags.
- The compile covers the whole editor assembly; ignore any pre-existing errors in unrelated files and focus on `SpritePivotBatchWindow.cs`.
- The `.rsp` references files by path, so csc reads current on-disk contents (no Unity save needed). If `ls` finds no `.rsp`, the user must let Unity compile once first; note it and proceed to the manual test.

## File structure

```
Assets/Game/Editor/SpriteTools/
  Core/                              ← UNCHANGED
    SpriteNaming.cs
    SpriteSheetRows.cs
    SpriteImportOps.cs
  UI/                               ← NEW folder
    SpritePivotBatchWindow.cs        ← EditorWindow controller
    SpritePivotBatchWindow.uxml      ← window layout
    SpritePivotBatchWindow.uss       ← styles
    RowView.uxml                     ← one-row template
  SpritePivotBatchTool.cs            ← DELETED (+ .meta)
```

---

## Task 1: Create the USS stylesheet

**Files:**
- Create: `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uss`

- [ ] **Step 1: Write the stylesheet**

Create `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uss`:

```css
.root {
    padding: 6px;
}

.section {
    margin-bottom: 6px;
}

.mode-banner {
    -unity-font-style: bold;
    margin-bottom: 4px;
}

.header {
    -unity-font-style: bold;
    margin-top: 2px;
    margin-bottom: 2px;
}

.indent {
    margin-left: 15px;
}

.row-line {
    flex-direction: row;
    align-items: center;
}

.prefix {
    width: 120px;
}

.unit {
    width: 90px;
}

.xy {
    width: 60px;
    margin-left: 2px;
}

.row-label {
    width: 110px;
}

.row-name {
    flex-grow: 1;
    min-width: 80px;
}

.row-preset {
    width: 120px;
}

.row-pivot-label {
    width: 40px;
}

.apply {
    margin-top: 4px;
    margin-bottom: 4px;
    height: 22px;
}

.preview-scroll {
    max-height: 180px;
    border-width: 1px;
    border-color: var(--unity-colors-helpbox-border);
}

.preview-line {
    flex-direction: row;
    justify-content: space-between;
}

.preview-left {
    flex-grow: 1;
}

.preview-right {
    -unity-text-align: middle-right;
    color: var(--unity-colors-default-text);
}

.preview-row-header {
    -unity-font-style: bold;
}
```

- [ ] **Step 2: Verify the file exists and is non-empty**

Run: `ls -l "Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uss"`
Expected: file listed with a non-zero size.

(No commit — see execution notes. USS is not compiled by csc; it is validated visually in the final smoke test.)

---

## Task 2: Create the row template UXML

**Files:**
- Create: `Assets/Game/Editor/SpriteTools/UI/RowView.uxml`

- [ ] **Step 1: Write the row template**

Create `Assets/Game/Editor/SpriteTools/UI/RowView.uxml`:

```xml
<ui:UXML xmlns:ui="UnityEngine.UIElements" xmlns:uie="UnityEditor.UIElements">
    <ui:VisualElement class="row-item">
        <ui:VisualElement class="row-line">
            <ui:Label name="row-label" class="row-label" />
            <ui:TextField name="row-name" class="row-name" />
        </ui:VisualElement>
        <ui:VisualElement name="row-pivot-line" class="row-line indent">
            <ui:Label text="Pivot" class="row-pivot-label" />
            <uie:EnumField name="row-preset" class="row-preset" />
            <uie:EnumField name="row-unit" class="unit" />
            <uie:FloatField name="row-x" label="X" class="xy" />
            <uie:FloatField name="row-y" label="Y" class="xy" />
        </ui:VisualElement>
    </ui:VisualElement>
</ui:UXML>
```

- [ ] **Step 2: Verify the file exists**

Run: `ls -l "Assets/Game/Editor/SpriteTools/UI/RowView.uxml"`
Expected: file listed.

---

## Task 3: Create the main window UXML

**Files:**
- Create: `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uxml`

- [ ] **Step 1: Write the window layout**

Create `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uxml`. Element `name`s here are exactly the ones queried in Task 4 — do not change them. The `<Style src=...>` pulls in the USS from Task 1 (same folder).

```xml
<ui:UXML xmlns:ui="UnityEngine.UIElements" xmlns:uie="UnityEditor.UIElements">
    <Style src="SpritePivotBatchWindow.uss" />
    <ui:VisualElement class="root">
        <ui:Label name="mode-label" text="Mode" class="mode-banner" />

        <ui:VisualElement class="section">
            <ui:Toggle name="change-pivot" label="Change Pivot" binding-path="changePivot" />
            <ui:VisualElement name="each-row-own-pivot" class="indent">
                <ui:Toggle label="Each row with own pivot" binding-path="eachRowOwnPivot" />
            </ui:VisualElement>
            <ui:Toggle name="change-names" label="Rename" binding-path="changeNames" />
        </ui:VisualElement>

        <ui:VisualElement name="pivot-section" class="section">
            <uie:EnumField name="pivot-preset" label="Pivot" binding-path="pivotPreset" />
            <ui:VisualElement name="custom-pivot-row" class="row-line">
                <ui:Label text="Custom Pivot" class="prefix" />
                <uie:EnumField name="pivot-unit" binding-path="unitMode" class="unit" />
                <uie:FloatField name="pivot-x" label="X" binding-path="customPivot.x" class="xy" />
                <uie:FloatField name="pivot-y" label="Y" binding-path="customPivot.y" class="xy" />
            </ui:VisualElement>
        </ui:VisualElement>

        <ui:VisualElement name="flat-name-section" class="section">
            <ui:TextField name="base-name" label="Base Name" binding-path="renameBaseName" />
        </ui:VisualElement>

        <ui:VisualElement name="rows-section" class="section">
            <ui:Label name="rows-header" text="Rows" class="header" />
            <ui:VisualElement name="rows-container" />
        </ui:VisualElement>

        <ui:VisualElement name="numbering-section" class="section">
            <uie:IntegerField name="start-index" label="Start Index" binding-path="renameStartIndex" />
            <uie:IntegerField name="pad-width" label="Pad Width" binding-path="renamePadWidth" />
        </ui:VisualElement>

        <ui:Button name="apply-button" text="Apply" class="apply" />

        <ui:VisualElement class="section">
            <ui:Label text="Preview" class="header" />
            <ui:ScrollView name="preview-scroll" class="preview-scroll">
                <ui:VisualElement name="preview-container" />
            </ui:ScrollView>
        </ui:VisualElement>

        <ui:HelpBox message-type="Info" text="Mode follows selection: Sprite sub-assets -> flat (selection-order) rename; a whole texture -> per-row.&#10;Custom pivot is interpreted from the bottom-left of each sprite rect, like Sprite Editor." />
    </ui:VisualElement>
</ui:UXML>
```

- [ ] **Step 2: Verify the file exists**

Run: `ls -l "Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.uxml"`
Expected: file listed.

---

## Task 4: Create the window controller (C#)

**Files:**
- Create: `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.cs`

- [ ] **Step 1: Write the controller**

Create `Assets/Game/Editor/SpriteTools/UI/SpritePivotBatchWindow.cs` with the complete content below. The selection/mode/Apply methods are ported from the old `SpritePivotBatchTool` (only the per-row state changes from three parallel lists to one `List<RowConfig>`).

```csharp
// SpritePivotBatchWindow.cs
// Sprite Tools — UI Toolkit batch pivot + naming window.

using System;
using System.Collections.Generic;
using System.IO;
using Game.Editor.SpriteTools.Core;
using UnityEditor;
using UnityEditor.UIElements;
using UnityEngine;
using UnityEngine.UIElements;

namespace Game.Editor.SpriteTools.UI {
    /// <summary>
    /// UI Toolkit front-end for the Sprite Tools batch operations. Loads a UXML layout, binds its static
    /// controls to this window's serialized fields, renders the dynamic per-row list by hand, and delegates
    /// every asset edit to <see cref="SpriteImportOps"/> / <see cref="SpriteSheetRows"/> /
    /// <see cref="SpriteNaming"/>. The window holds UI state only; it contains no import logic.
    /// </summary>
    public sealed class SpritePivotBatchWindow : EditorWindow {
        private enum PivotUnitMode {
            Normalized,
            Pixels
        }

        /// <summary>How the current selection is interpreted.</summary>
        private enum RenameMode {
            None,
            Flat,
            PerRow
        }

        /// <summary>Per-row UI state: replaces the old three index-parallel lists with one typed entry.</summary>
        [Serializable]
        private sealed class RowConfig {
            public string name;
            public SpriteAlignment alignment;
            public Vector2 customPivot;
        }

        /// <summary>One selected Sprite sub-asset, captured in Selection order for flat rename/preview.</summary>
        private readonly struct SelectedSprite {
            public readonly string assetPath;
            public readonly string currentName;

            public SelectedSprite(string assetPath, string currentName) {
                this.assetPath = assetPath;
                this.currentName = currentName;
            }
        }

        // UI assets are loaded by path: the window is code-instantiated, so there is no inspector to wire
        // serialized VisualTreeAsset references on.
        private const string UiFolder = "Assets/Game/Editor/SpriteTools/UI/";
        private const string WindowUxmlPath = UiFolder + "SpritePivotBatchWindow.uxml";
        private const string RowUxmlPath = UiFolder + "RowView.uxml";

        // --- Serialized state (bound via SerializedObject / binding-path) --------------------------------
        [SerializeField]
        private bool changePivot = true;

        [SerializeField]
        private SpriteAlignment pivotPreset = SpriteAlignment.Center;

        [SerializeField]
        private PivotUnitMode unitMode = PivotUnitMode.Pixels;

        [SerializeField]
        private Vector2 customPivot = new Vector2(16f, 0f);

        [SerializeField]
        private bool eachRowOwnPivot;

        [SerializeField]
        private bool changeNames;

        [SerializeField]
        private string renameBaseName = "sprite";

        [SerializeField]
        private int renameStartIndex;

        [SerializeField]
        private int renamePadWidth = 2;

        [SerializeField]
        private List<RowConfig> rows = new List<RowConfig>();

        // Per-row cell cache, rebuilt only when the selected texture changes (row detection loads sub-assets).
        private string cachedRowTexturePath;
        private List<List<SpriteCell>> cachedRows;

        // --- UI element references ----------------------------------------------------------------------
        private SerializedObject serialized;
        private VisualTreeAsset rowTemplate;
        private Label modeLabel;
        private VisualElement eachRowOwnPivotContainer;
        private VisualElement pivotSection;
        private EnumField pivotPresetField;
        private VisualElement customPivotRow;
        private VisualElement flatNameSection;
        private VisualElement rowsSection;
        private VisualElement rowsContainer;
        private VisualElement numberingSection;
        private IntegerField padWidthField;
        private Button applyButton;
        private VisualElement previewContainer;

        private bool IsPixels => unitMode == PivotUnitMode.Pixels;

        [MenuItem("Tools/Sprites/Batch Pivot (Sprite Editor Style)")]
        public static void Open() {
            var wnd = GetWindow<SpritePivotBatchWindow>("Batch Sprite Pivot");
            wnd.minSize = new Vector2(480f, 440f);
            wnd.Show();
        }

        private void CreateGUI() {
            var tree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(WindowUxmlPath);
            if (tree == null) {
                rootVisualElement.Add(new HelpBox($"Missing UXML at {WindowUxmlPath}", HelpBoxMessageType.Error));
                return;
            }

            tree.CloneTree(rootVisualElement);
            rowTemplate = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(RowUxmlPath);

            QueryElements();

            serialized = new SerializedObject(this);
            rootVisualElement.Bind(serialized);

            // One callback drives every live update when a bound control changes.
            rootVisualElement.TrackSerializedObjectValue(serialized, _ => Refresh());

            // Pad width is non-negative, mirroring the old Mathf.Max(0, ...) clamp.
            // Clamp without re-notifying (setting .value here would re-enter this callback).
            padWidthField.RegisterValueChangedCallback(evt => {
                if (evt.newValue < 0) {
                    padWidthField.SetValueWithoutNotify(0);
                    renamePadWidth = 0;
                    serialized.Update();
                }
            });

            applyButton.clicked += OnApplyClicked;
            Refresh();
        }

        private void QueryElements() {
            modeLabel = rootVisualElement.Q<Label>("mode-label");
            eachRowOwnPivotContainer = rootVisualElement.Q<VisualElement>("each-row-own-pivot");
            pivotSection = rootVisualElement.Q<VisualElement>("pivot-section");
            pivotPresetField = rootVisualElement.Q<EnumField>("pivot-preset");
            customPivotRow = rootVisualElement.Q<VisualElement>("custom-pivot-row");
            flatNameSection = rootVisualElement.Q<VisualElement>("flat-name-section");
            rowsSection = rootVisualElement.Q<VisualElement>("rows-section");
            rowsContainer = rootVisualElement.Q<VisualElement>("rows-container");
            numberingSection = rootVisualElement.Q<VisualElement>("numbering-section");
            padWidthField = rootVisualElement.Q<IntegerField>("pad-width");
            applyButton = rootVisualElement.Q<Button>("apply-button");
            previewContainer = rootVisualElement.Q<VisualElement>("preview-container");
        }

        private void OnSelectionChange() {
            Refresh();
        }

        // --- View synchronization -----------------------------------------------------------------------

        // Single source of truth for derived view state: mode banner, section visibility/enable, the rows
        // list, and the live preview. Safe to call before the UI exists (early Selection callbacks).
        private void Refresh() {
            if (modeLabel == null) {
                return;
            }

            var mode = DetermineMode(out var perRowPath);
            if (mode == RenameMode.PerRow) {
                EnsureRowCache(perRowPath);
            }

            var rowOwnPivot = mode == RenameMode.PerRow && changePivot && eachRowOwnPivot;

            modeLabel.text = ModeText(mode, perRowPath);

            SetDisplay(eachRowOwnPivotContainer, mode == RenameMode.PerRow && changePivot);

            // Shared pivot block: hidden in per-row "own pivot" mode (each row carries its own).
            var showSharedPivot = mode != RenameMode.None && changePivot
                && !(mode == RenameMode.PerRow && eachRowOwnPivot);
            SetDisplay(pivotSection, showSharedPivot);
            pivotPresetField.label = mode == RenameMode.PerRow ? "Sheet Pivot" : "Pivot";
            customPivotRow.SetEnabled(pivotPreset == SpriteAlignment.Custom);

            SetDisplay(flatNameSection, mode == RenameMode.Flat && changeNames);

            var showRows = mode == RenameMode.PerRow && (changeNames || rowOwnPivot);
            SetDisplay(rowsSection, showRows);
            if (showRows) {
                RebuildRows(rowOwnPivot);
            }

            SetDisplay(numberingSection, mode != RenameMode.None && changeNames);

            applyButton.SetEnabled((changePivot || changeNames) && mode != RenameMode.None);

            RebuildPreview(mode, rowOwnPivot);
        }

        private static void SetDisplay(VisualElement element, bool visible) {
            element.style.display = visible ? DisplayStyle.Flex : DisplayStyle.None;
        }

        // Renders one cloned RowView per detected row. Name/X/Y/unit edits only refresh the preview so the
        // focused field is not destroyed mid-edit; a preset change calls Refresh to re-toggle enable states.
        private void RebuildRows(bool rowOwnPivot) {
            rowsContainer.Clear();
            if (cachedRows == null || rowTemplate == null) {
                return;
            }

            var count = Mathf.Min(cachedRows.Count, rows.Count);
            for (int r = 0; r < count; r++) {
                var index = r;
                var cfg = rows[index];

                var rowElem = rowTemplate.CloneTree();
                var label = rowElem.Q<Label>("row-label");
                var nameField = rowElem.Q<TextField>("row-name");
                var pivotLine = rowElem.Q<VisualElement>("row-pivot-line");
                var presetField = rowElem.Q<EnumField>("row-preset");
                var unitField = rowElem.Q<EnumField>("row-unit");
                var xField = rowElem.Q<FloatField>("row-x");
                var yField = rowElem.Q<FloatField>("row-y");

                label.text = $"Row {index} ({cachedRows[index].Count})";

                nameField.SetEnabled(changeNames);
                nameField.SetValueWithoutNotify(cfg.name);
                nameField.RegisterValueChangedCallback(evt => {
                    rows[index].name = evt.newValue;
                    RefreshPreview();
                });

                SetDisplay(pivotLine, rowOwnPivot);
                if (rowOwnPivot) {
                    presetField.Init(cfg.alignment);
                    presetField.SetValueWithoutNotify(cfg.alignment);
                    presetField.RegisterValueChangedCallback(evt => {
                        rows[index].alignment = (SpriteAlignment)evt.newValue;
                        Refresh();
                    });

                    var customEnabled = cfg.alignment == SpriteAlignment.Custom;

                    unitField.Init(unitMode);
                    unitField.SetValueWithoutNotify(unitMode);
                    unitField.SetEnabled(customEnabled);
                    unitField.RegisterValueChangedCallback(evt => {
                        // unitMode is shared by all rows, so rebuild every row to keep the
                        // sibling unit fields in sync (full Refresh, not just the preview).
                        unitMode = (PivotUnitMode)evt.newValue;
                        serialized.Update();
                        Refresh();
                    });

                    xField.SetEnabled(customEnabled);
                    yField.SetEnabled(customEnabled);
                    xField.SetValueWithoutNotify(cfg.customPivot.x);
                    yField.SetValueWithoutNotify(cfg.customPivot.y);
                    xField.RegisterValueChangedCallback(evt => {
                        rows[index].customPivot = new Vector2(evt.newValue, rows[index].customPivot.y);
                        RefreshPreview();
                    });
                    yField.RegisterValueChangedCallback(evt => {
                        rows[index].customPivot = new Vector2(rows[index].customPivot.x, evt.newValue);
                        RefreshPreview();
                    });
                }

                rowsContainer.Add(rowElem);
            }
        }

        // Recomputes mode and refreshes only the preview (used by row edits that must not rebuild the rows).
        private void RefreshPreview() {
            var mode = DetermineMode(out _);
            var rowOwnPivot = mode == RenameMode.PerRow && changePivot && eachRowOwnPivot;
            RebuildPreview(mode, rowOwnPivot);
        }

        private void RebuildPreview(RenameMode mode, bool rowOwnPivot) {
            previewContainer.Clear();

            if (mode == RenameMode.None) {
                previewContainer.Add(MakePreviewLine("—", string.Empty));
                return;
            }

            if (mode == RenameMode.Flat) {
                previewContainer.Add(MakePreviewLine("Pivot",
                    changePivot ? PivotPreviewText(pivotPreset, customPivot) : "(unchanged)"));

                var selected = CollectSelectedSpritesOrdered();
                for (int i = 0; i < selected.Count; i++) {
                    previewContainer.Add(MakePreviewLine(selected[i].currentName,
                        changeNames ? "→  " + BuildName(renameBaseName, i) : "(unchanged)"));
                }

                return;
            }

            if (cachedRows == null) {
                return;
            }

            var count = Mathf.Min(cachedRows.Count, rows.Count);
            for (int r = 0; r < count; r++) {
                string pivotText;
                if (!changePivot) {
                    pivotText = "(unchanged)";
                } else if (rowOwnPivot) {
                    pivotText = PivotPreviewText(rows[r].alignment, rows[r].customPivot);
                } else {
                    pivotText = PivotPreviewText(pivotPreset, customPivot);
                }

                var header = MakePreviewLine($"Row {r} — {rows[r].name}", pivotText);
                header.AddToClassList("preview-row-header");
                previewContainer.Add(header);

                var row = cachedRows[r];
                for (int c = 0; c < row.Count; c++) {
                    previewContainer.Add(MakePreviewLine("    " + row[c].Name,
                        changeNames ? "→  " + BuildName(rows[r].name, c) : "(unchanged)"));
                }
            }
        }

        private static VisualElement MakePreviewLine(string left, string right) {
            var line = new VisualElement();
            line.AddToClassList("preview-line");

            var leftLabel = new Label(left);
            leftLabel.AddToClassList("preview-left");
            line.Add(leftLabel);

            var rightLabel = new Label(right);
            rightLabel.AddToClassList("preview-right");
            line.Add(rightLabel);

            return line;
        }

        private string PivotPreviewText(SpriteAlignment preset, Vector2 custom) {
            if (preset != SpriteAlignment.Custom) {
                return preset.ToString();
            }

            var unit = IsPixels ? "px" : "normalized";
            return $"Custom ({custom.x}, {custom.y}) {unit}";
        }

        private string BuildName(string baseName, int offset) {
            return SpriteNaming.BuildIndexedName(baseName, renameStartIndex, offset, renamePadWidth);
        }

        // --- Apply --------------------------------------------------------------------------------------

        private void OnApplyClicked() {
            var mode = DetermineMode(out var perRowPath);
            Apply(mode, perRowPath);
            Refresh();
        }

        private void Apply(RenameMode mode, string perRowPath) {
            // Pivot first: rename changes sprite names, which would otherwise break the pivot pass's
            // name-based matching.
            if (changePivot) {
                if (mode == RenameMode.PerRow && eachRowOwnPivot) {
                    ApplyPerRowPivot(perRowPath);
                } else {
                    ApplyUniformPivot();
                }
            }

            if (changeNames) {
                if (mode == RenameMode.Flat) {
                    RenameFlat();
                } else if (mode == RenameMode.PerRow) {
                    RenamePerRow(perRowPath);
                } else {
                    Debug.LogWarning("Rename skipped: select Sprite sub-assets (flat) or a whole texture (per-row).");
                }

                // Names changed on disk; drop the row cache so the UI/preview rebuild from the new names.
                cachedRowTexturePath = null;
            }
        }

        // Flat selection (filter by selected sprite names) or whole-texture selection (all sprites).
        private void ApplyUniformPivot() {
            var targets = CollectSelection();
            if (targets.Count == 0) {
                Debug.LogWarning("No valid Sprite textures/sprites selected.");
                return;
            }

            var changed = 0;
            var skipped = 0;

            AssetDatabase.StartAssetEditing();
            try {
                foreach (var kv in targets) {
                    if (SpriteImportOps.ApplyUniformPivot(kv.Key, pivotPreset, customPivot, IsPixels, kv.Value)) {
                        changed++;
                    } else {
                        skipped++;
                    }
                }
            }
            finally {
                AssetDatabase.StopAssetEditing();
            }

            Debug.Log($"Batch pivot finished. Changed: {changed}, skipped: {skipped}");
        }

        private void ApplyPerRowPivot(string texturePath) {
            EnsureRowCache(texturePath);
            if (cachedRows == null || cachedRows.Count == 0) {
                Debug.LogWarning("Per-row pivot skipped: no rows detected.");
                return;
            }

            var pivotByName = new Dictionary<string, PivotSetting>();
            var count = Mathf.Min(cachedRows.Count, rows.Count);
            for (int r = 0; r < count; r++) {
                var setting = new PivotSetting(rows[r].alignment, rows[r].customPivot);
                var row = cachedRows[r];
                for (int c = 0; c < row.Count; c++) {
                    pivotByName[row[c].Name] = setting;
                }
            }

            AssetDatabase.StartAssetEditing();
            try {
                SpriteImportOps.ApplyPivots(texturePath, pivotByName, IsPixels);
            }
            finally {
                AssetDatabase.StopAssetEditing();
            }

            Debug.Log($"Per-row pivot applied to {cachedRows.Count} rows.");
        }

        private void RenameFlat() {
            if (string.IsNullOrEmpty(renameBaseName)) {
                Debug.LogWarning("Rename skipped: Base Name is empty.");
                return;
            }

            var selected = CollectSelectedSpritesOrdered();
            if (selected.Count == 0) {
                Debug.LogWarning("Rename skipped: no Sprite sub-assets selected.");
                return;
            }

            // Group renames per texture while numbering from the global selection order, so a selection
            // within one sheet reads as one continuous sequence.
            var renamesByPath = new Dictionary<string, Dictionary<string, string>>();
            for (int i = 0; i < selected.Count; i++) {
                var newName = BuildName(renameBaseName, i);
                if (!renamesByPath.TryGetValue(selected[i].assetPath, out var map)) {
                    map = new Dictionary<string, string>();
                    renamesByPath[selected[i].assetPath] = map;
                }

                map[selected[i].currentName] = newName;
            }

            RunRenames(renamesByPath);
        }

        private void RenamePerRow(string texturePath) {
            EnsureRowCache(texturePath);
            if (cachedRows == null || cachedRows.Count == 0) {
                Debug.LogWarning("Rename skipped: no rows detected on the selected texture.");
                return;
            }

            var map = new Dictionary<string, string>();
            var count = Mathf.Min(cachedRows.Count, rows.Count);
            for (int r = 0; r < count; r++) {
                var rowName = rows[r].name;
                if (string.IsNullOrEmpty(rowName)) {
                    Debug.LogWarning($"Rename skipped: Row {r} name is empty.");
                    return;
                }

                var row = cachedRows[r];
                for (int c = 0; c < row.Count; c++) {
                    // Numbering restarts per row, so every row reads as <rowName>_00.._0N left-to-right.
                    map[row[c].Name] = BuildName(rowName, c);
                }
            }

            RunRenames(new Dictionary<string, Dictionary<string, string>> { { texturePath, map } });
        }

        private static void RunRenames(Dictionary<string, Dictionary<string, string>> renamesByPath) {
            var changed = 0;
            var skipped = 0;

            AssetDatabase.StartAssetEditing();
            try {
                foreach (var kv in renamesByPath) {
                    if (SpriteImportOps.Rename(kv.Key, kv.Value)) {
                        changed++;
                    } else {
                        skipped++;
                    }
                }
            }
            finally {
                AssetDatabase.StopAssetEditing();
            }

            Debug.Log($"Batch rename finished. Textures changed: {changed}, skipped: {skipped}");
        }

        // --- Selection / row cache ----------------------------------------------------------------------

        private static RenameMode DetermineMode(out string perRowTexturePath) {
            perRowTexturePath = null;

            // Explicitly selected sub-sprites take priority: that is the flat case.
            if (CollectSelectedSpritesOrdered().Count > 0) {
                return RenameMode.Flat;
            }

            var path = GetSelectedTexturePath();
            if (!string.IsNullOrEmpty(path)) {
                perRowTexturePath = path;
                return RenameMode.PerRow;
            }

            return RenameMode.None;
        }

        private static string ModeText(RenameMode mode, string perRowPath) {
            switch (mode) {
                case RenameMode.Flat:
                    return $"Flat selection ({CollectSelectedSpritesOrdered().Count} sprites)";
                case RenameMode.PerRow:
                    return $"Per-row — {Path.GetFileName(perRowPath)}";
                default:
                    return "Nothing selected";
            }
        }

        // Selected Sprite sub-assets in Selection order (whole-texture selections are ignored here: they
        // have no per-sprite order and instead drive per-row mode).
        private static List<SelectedSprite> CollectSelectedSpritesOrdered() {
            var result = new List<SelectedSprite>();

            var objects = Selection.objects;
            if (objects == null || objects.Length == 0) {
                return result;
            }

            for (int i = 0; i < objects.Length; i++) {
                var sprite = objects[i] as Sprite;
                if (sprite == null) {
                    continue;
                }

                var path = AssetDatabase.GetAssetPath(sprite);
                if (string.IsNullOrEmpty(path)) {
                    continue;
                }

                result.Add(new SelectedSprite(path, sprite.name));
            }

            return result;
        }

        // Pivot targets: each selected texture/sprite path mapped to the selected sprite names (null = whole sheet).
        private static Dictionary<string, HashSet<string>> CollectSelection() {
            var result = new Dictionary<string, HashSet<string>>();

            var objects = Selection.objects;
            if (objects == null || objects.Length == 0) {
                return result;
            }

            for (int i = 0; i < objects.Length; i++) {
                var obj = objects[i];
                if (obj == null) {
                    continue;
                }

                var path = AssetDatabase.GetAssetPath(obj);
                if (string.IsNullOrEmpty(path)) {
                    continue;
                }

                var sprite = obj as Sprite;
                if (sprite != null) {
                    if (!result.TryGetValue(path, out var set) || set == null) {
                        set = new HashSet<string>();
                        result[path] = set;
                    }

                    set.Add(sprite.name);
                    continue;
                }

                var tex = obj as Texture2D;
                if (tex != null) {
                    // A whole-texture selection: null set means "all sprites in the sheet".
                    if (!result.ContainsKey(path)) {
                        result[path] = null;
                    }
                }
            }

            return result;
        }

        // The whole-texture selection for per-row mode: prefer the active object, else the first selected texture.
        private static string GetSelectedTexturePath() {
            var active = Selection.activeObject as Texture2D;
            if (active != null) {
                var p = AssetDatabase.GetAssetPath(active);
                if (!string.IsNullOrEmpty(p)) {
                    return p;
                }
            }

            var objects = Selection.objects;
            if (objects != null) {
                for (int i = 0; i < objects.Length; i++) {
                    var tex = objects[i] as Texture2D;
                    if (tex != null) {
                        var p = AssetDatabase.GetAssetPath(tex);
                        if (!string.IsNullOrEmpty(p)) {
                            return p;
                        }
                    }
                }
            }

            return null;
        }

        // Rebuilds the per-row cache only when the target texture changes (row detection loads all sub-assets).
        // A (re)built sheet gets fresh suggestions: each row name defaults to its sprites' shared prefix, and
        // each row's pivot defaults to the current global pivot.
        private void EnsureRowCache(string texturePath) {
            if (texturePath == cachedRowTexturePath && cachedRows != null) {
                return;
            }

            cachedRowTexturePath = texturePath;
            cachedRows = SpriteSheetRows.Detect(texturePath);

            rows.Clear();
            for (int r = 0; r < cachedRows.Count; r++) {
                rows.Add(new RowConfig {
                    name = SpriteSheetRows.DeriveRowName(cachedRows[r]),
                    alignment = pivotPreset,
                    customPivot = customPivot
                });
            }
        }
    }
}
```

- [ ] **Step 2: Offline compile-check**

Run the **Reusable offline compile-check** block from the top of this plan.
Expected: no errors referencing `SpritePivotBatchWindow.cs`. (Pre-existing errors in unrelated files, if any, can be ignored — but there should be none introduced by this file.)

If errors mention `SpritePivotBatchTool.cs` still existing, that is expected until Task 5 (the `sed` already excludes it from the clone; the file is deleted on disk in Task 5).

---

## Task 5: Delete the old IMGUI window

**Files:**
- Delete: `Assets/Game/Editor/SpriteTools/SpritePivotBatchTool.cs`
- Delete: `Assets/Game/Editor/SpriteTools/SpritePivotBatchTool.cs.meta`

- [ ] **Step 1: Remove the old window and its meta with git**

Run:
```bash
cd "F:/Work/unity/piratik/PlatformStudy"
git rm "Assets/Game/Editor/SpriteTools/SpritePivotBatchTool.cs" \
       "Assets/Game/Editor/SpriteTools/SpritePivotBatchTool.cs.meta"
```
Expected: both files staged for deletion. (Removing the `.meta` with the script keeps Unity's asset bookkeeping consistent.)

- [ ] **Step 2: Confirm the menu item is not duplicated**

Run: `grep -rn "Tools/Sprites/Batch Pivot" Assets/Game/Editor/SpriteTools`
Expected: exactly one match, in `UI/SpritePivotBatchWindow.cs`.

- [ ] **Step 3: Re-run the offline compile-check**

Run the **Reusable offline compile-check** block again.
Expected: clean compile (no `SpritePivotBatchTool` / `SpritePivotBatchWindow` errors). The old file is now gone from disk, so no stale-reference errors remain.

---

## Task 6: Manual verification in Unity (user step) and commit

This task requires the Unity Editor and is performed/confirmed by the user. It generates the `.meta` files for the new assets and validates runtime behavior.

- [ ] **Step 1: Import new assets**

Focus the Unity Editor window so it imports the new `UI/` folder. Unity generates `.meta` files for `SpritePivotBatchWindow.cs`, `SpritePivotBatchWindow.uxml`, `SpritePivotBatchWindow.uss`, `RowView.uxml`, and the `UI` folder. Confirm the Console shows no compile errors.

- [ ] **Step 2: Open the window**

Menu: `Tools/Sprites/Batch Pivot (Sprite Editor Style)`.
Expected: the window opens titled "Batch Sprite Pivot", showing "Nothing selected", greyed Apply, and the info help box.

- [ ] **Step 3: Flat-mode smoke test**

In the Project window, select two or more Sprite sub-assets from a Multiple-mode sheet.
Expected: banner reads `Flat selection (N sprites)`. With **Change Pivot** on, set preset to `Custom`, confirm the unit + X/Y row enables and the preview "Pivot" line updates live. With **Rename** on, type a Base Name and confirm preview shows `current →  base_00, base_01, ...` updating as you type. Click **Apply**; confirm the Console logs the pivot/rename summaries and the sprite names/pivots changed in the Sprite Editor.

- [ ] **Step 4: Per-row-mode smoke test**

Select a whole multi-row Multiple-mode texture (the Texture2D asset, not its sub-sprites).
Expected: banner reads `Per-row — <file>`. The Rows list shows one line per detected row with a suggested name. Toggle **Each row with own pivot** and confirm each row gets a pivot line (preset + unit + X/Y, enabled only when preset is `Custom`). Edit a couple of row names and confirm the preview updates without losing focus. Click **Apply**; confirm per-row names/pivots applied and that references to the sprites still resolve (no pink/missing sprites).

- [ ] **Step 5: Review and commit (user)**

Review the diff (new `UI/` files + their Unity-generated `.meta` files, deleted `SpritePivotBatchTool.cs` + `.meta`). When satisfied, commit. Suggested message:
```
Tools: Sprite Tools - rewrite batch pivot window in UI Toolkit
```

---

## Self-review

- **Spec coverage:** file layout (Tasks 1–5) ✓; replace IMGUI entirely (Task 5) ✓; UXML+USS+C# (Tasks 1–4) ✓; hybrid binding — SerializedObject for static controls (Task 3 `binding-path`, Task 4 `Bind`) + manual rows (Task 4 `RebuildRows`) ✓; faithful Apply ordering & Core calls (Task 4 ports) ✓; UX polish — live preview + reactive enable-states (Task 4 `Refresh`/`RefreshPreview`) ✓; theme-aware USS variables (Task 1) ✓; compile-check + manual smoke test (Tasks 4/5/6) ✓.
- **Placeholders:** none — full file contents and exact commands provided.
- **Type consistency:** `RowConfig{name,alignment,customPivot}`, `RenameMode`, `PivotUnitMode`, element names (`mode-label`, `pivot-preset`, `custom-pivot-row`, `flat-name-section`, `rows-container`, `pad-width`, `apply-button`, `preview-container`, `row-label`/`row-name`/`row-pivot-line`/`row-preset`/`row-unit`/`row-x`/`row-y`) match between the UXML (Tasks 2–3) and the C# queries/callbacks (Task 4). `binding-path` values (`changePivot`, `eachRowOwnPivot`, `changeNames`, `pivotPreset`, `unitMode`, `customPivot.x/.y`, `renameBaseName`, `renameStartIndex`, `renamePadWidth`) match the serialized fields. Core signatures (`SpriteImportOps.ApplyUniformPivot/ApplyPivots/Rename`, `SpriteSheetRows.Detect/DeriveRowName`, `SpriteNaming.BuildIndexedName`, `PivotSetting`, `SpriteCell`) match the unchanged Core.

## Risks / follow-ups

- **EnumField binding in UXML:** binding a `binding-path` `EnumField` to an enum `SerializedProperty` is expected to work in 2022.3; if the pivot/unit dropdowns show empty in the smoke test, the fallback is to drop their `binding-path` and wire them with `Init` + a `RegisterValueChangedCallback` in `CreateGUI` (same pattern already used for the per-row dropdowns).
- **Window state reset:** renaming the type `SpritePivotBatchTool` → `SpritePivotBatchWindow` resets persisted window position/size only (no asset data).
- `.meta` GUIDs are created by Unity at import (Task 6 Step 1); do not author them by hand.
